...
class HikesSection extends ObjectSection {
    constructor(props) {
        ...
        this.afterCoordinatesToHistoricalWeather = this.afterCoordinatesToHistoricalWeather.bind(this);
    }
    ...
    zipCodeToCoordinate(hike, callback){
         ...
         * @returns None
         */
        const ZIP_CODE_TO_COORDINATE_URL = `http://api.openweathermap.org/geo/1.0/zip?zip=${hike.zipcode},us&appid=${process.env.REACT_APP_WEATHER_API_KEY}`;
    ...
    afterZipCodesToCoordinates(error, coordinates){
         ...
         */
        if(!error){
            async.map(coordinates, this.coordinateToHistoricalWeather, this.afterCoordinatesToHistoricalWeather);
        }
    }
    ...
    coordinateToHistoricalWeather(coordinate, callback){
         ...
         * @returns None
         */
        const HISTORICAL_WEATHER_URL = `https://api.openweathermap.org/data/2.5/onecall?lat=${coordinate.latitude}&lon=${coordinate.longitude}&exclude=current,minutely,hourly,alert&units=imperial&appid=${process.env.REACT_APP_WEATHER_API_KEY}`;
