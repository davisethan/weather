...
class CrimesSection extends ObjectSection {
  ...
  mapCrime(crime, callback){
    ...
    if(!crime){
      ...
      const HISTORICAL_WEATHER_URL = `https://api.openweathermap.org/data/2.5/onecall?lat=${crime.latitude}&lon=${crime.longitude}&exclude=current,minutely,hourly,alert&units=imperial&appid=${process.env.REACT_APP_WEATHER_API_KEY}`;
